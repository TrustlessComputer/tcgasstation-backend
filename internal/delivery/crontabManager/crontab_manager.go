...
func (h *CrontabManager) StartServer() {
	...
	for _, item := range eventList {
		...
		if err != nil {
			...
			// TODO: notify
			logger.AtLog.Info(fmt.Sprintf("\nHas error--> %v\n", err))
			go h.trackCronJobManagerLogs(fmt.Sprintf("[%v-%v]", item.Group, item.JobName), "Cron job manager -> StartServer", item.CollectionName(), item.Enabled, "h.Validation(item) - with err", err.Error())
			continue
		}
	...
	idEntries, err := c.AddFunc(cronJobManager.Schedule, func() {
		...
		defer func() {
			if err := recover(); err != nil {
				logger.AtLog.Error(fmt.Sprintf("\nError (defer) calling %s->%s: %v", eventName, cronJobManager.FunctionName, err))
				go h.trackCronJobManagerLogs(fmt.Sprintf("[%v-%v]", eventItem.Group, eventItem.JobName), "Cron job manager -> AddTask", eventItem.CollectionName(), cronJobManager.FunctionName, "call the function - with err", err)
				return
			}
		}()
		...
		logger.AtLog.Info(fmt.Sprintf("\nEnd %v: %v \n", cronJobManager.FunctionName, duration.String()))

		if err != nil {
			logger.AtLog.Info(fmt.Sprintf("TaskName %v, jobFunction %v, err %v\n", eventName, cronJobManager.FunctionName, err))

			return
		}

		logger.AtLog.Info(fmt.Sprintf("=======TaskName %v, jobFunction %v successfully=======\n", eventName, cronJobManager.FunctionName))
	})
