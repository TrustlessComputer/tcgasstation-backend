...
func GetBlockCountFromQuickNode(qn string) (uint64, error) {
	var getBlockCountResp struct {
	...
	fmt.Printf("body: %s\n ", body)
	err = json.Unmarshal(body, &getBlockCountResp)
	if err != nil {
		return 0, err
	}

	return getBlockCountResp.Result, getBlockCountResp.Error
}
